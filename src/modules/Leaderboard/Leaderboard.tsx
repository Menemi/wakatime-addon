...
const Leaderboard: React.FC<LeaderboardProps> = ({tableCode, onMembersChange, onTimeChange, onLoad}) => {
    ...
    const skeleton = Array.from({length: skeletonRowsNumber}).map((_, key) => (
        <tr key={key} className={styles.skeletonRow}>
            {Array.from({length: 6}).map((__, idx) => <td key={idx} className={styles.skeletonCell}>
    ...
    const table = data.map((row: LeaderboardRow, key: number) => <tr key={key} className={styles.row}>
        <td className={styles.cell}>{row.number}</td>
        <td className={styles.cell}>{row.username}</td>
        <td className={styles.cell}>
            {timeToString(row.currentWeekCodeTime)}
        </td>
        <td className={styles.cell}>
            {timeToString(row.avgDayCodeTime)}
        </td>
        <td className={styles.cell}>
    ...
    return <div id='leaderboard' className={styles.container}>
        <div className={styles.titleContainer}><h2 className={styles.title}>Текущая неделя</h2></div>
